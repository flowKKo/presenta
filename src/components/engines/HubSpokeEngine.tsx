  ...
  return (
    ...
    >
      ...
      <motion.div variants={motionConfig.child} className="flex-1 min-h-0 w-full">
        ...
        >
          {/* Orbit ellipse (orbit variant) */}
          {variant === 'orbit' && (
            <ellipse
              cx={cx} cy={cy} rx={orbitRx} ry={orbitRy}
              fill="none" stroke={colors.border} strokeWidth="1.5" strokeDasharray="8 5"
            />
          )}
              ...
              <g key={i}>
                ...
                {spoke.description && (
                  <text
                    x={sx} y={sy + 14}
                    textAnchor="middle" dominantBaseline="middle"
                    fontSize="11" fill="white" opacity={0.8}
                  >
                    {spoke.description}
                  </text>
                )}
