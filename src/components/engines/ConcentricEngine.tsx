...
export default function ConcentricEngine({ title, body, rings, variant, titleSize, bodySize, titleColor, textColor, colorPalette }: ConcentricSlideData) {
  return (
    <motion.div
    ...
    >
      ...
      <motion.div variants={motionConfig.child} className="flex-1 min-h-0 w-full">
        <ConcentricDiagram rings={rings} variant={variant} textColor={textColor} colorPalette={colorPalette} />
      </motion.div>
    </motion.div>
  )
}
