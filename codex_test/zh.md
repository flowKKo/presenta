...
一道简单的数学题：一个 10 步任务，每步工具调用的可靠性从 90% 提到 99%，全程成功率从 34.9% 跳到 90.4%。

Factory Droid 走极简路线 [8]：严格限制工具数量，简化输入 schema，为不同模型创建定制的工具变体。复杂 schema 导致错误率指数增长，且错误在多步任务中级联放大。这个策略在 Terminal-Bench 上得到了验证：Droid 用四个不同模型提交，全部进入前 15 名。
...
**强制显式规划**。Warp 在每个任务开始前，用 Opus 或 o3 生成 Todo 列表 [5]。他们试过条件式规划（让 agent 自己决定是否需要规划），发现不如强制规划。也试过用 extended thinking 替代显式规划，同样被放弃。Junie CLI 也是强制规划，生成 plan.md 后才开始编码 [12]。Droid 的规划工具把更新后的计划放在上下文窗口最末端，配合递近偏差保持方向感 [8]。
...
项目根目录放一个 Markdown 文件已经成了事实标准：CLAUDE.md [3]、AGENTS.md [4]、GEMINI.md [13]、guidelines.md [12]，用自然语言告诉 agent 项目的约定和禁忌，通过 git 共享给团队。差异在层级深度上：Claude Code 支持四级覆盖（组织级到目录级）[3]，Codex CLI 支持 root-to-leaf 逐级加载加本地 override [4]，Gemini CLI 相对简单 [13]。多数框架的记忆是无状态的，每次会话重新读取配置文件。OpenHands 是例外，用事件溯源架构做完整重放 [14]。
